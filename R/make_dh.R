...
make_dh <- function(indv,c=10,g_map){
  ...
  for(i in 1:c){
    ...
    new_dh@chromlist[[c]]=chrom_init(i,g_map,h1_breakpoints=h@breakpoints,h1_donors = h@donors, h2_breakpoints = h@breakpoints, h2_donors = h@donors)
    new_dh@chromlist[[c]]@h1@xo_no=length(new_dh[c]@h1@breakpoints-1)
    new_dh@chromlist[[c]]@h2@xo_no=length(new_dh[c]@h2@breakpoints-1)
  }
